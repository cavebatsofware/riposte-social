...
export default function Categories() {
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const r = await fetchApi("/api/categories");
  ...
  async function handleSave(id) {
    ...
    try {
      const r = await fetchApi(`/api/categories/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
  ...
  async function handleDelete(id) {
    ...
    setSuccess("");
    try {
      const r = await fetchApi(`/api/categories/${id}`, { method: "DELETE" });
...
function MemberModal({ category, onClose }) {
    ...
    async function load() {
      ...
      try {
        const [m, p] = await Promise.all([
          fetchApi(`/api/categories/${category.id}/members`),
          fetchApi("/api/profiles?limit=200"),
        ]);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const r = await fetchApi(`/api/categories/${category.id}/members`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
