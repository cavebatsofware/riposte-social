...
function Imports() {
  ...
  const fetchJobs = useCallback(async () => {
    try {
      const response = await fetchApi("/api/admin/imports?per_page=25");
  ...
  useEffect(() => {
    ...
    async function tick() {
      try {
        const response = await fetchApi(`/api/admin/imports/${selectedId}`);
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      const response = await fetchApi("/api/admin/imports/facebook", {
        method: "POST",
        body: form,
      });
      if (!response.ok) {
